# GitHub Release v1.0.1 생성 가이드

BS 로고가 적용된 새 버전 릴리즈를 생성하는 방법입니다.
...
1. 최신 워크플로우 클릭 ("feat: Update app icon and splash screen with BS logo")
...
- ✅ BS 로고 앱 아이콘 적용
- ✅ BS 로고 스플래시 화면 적용
- ✅ 모든 해상도 지원 (ldpi ~ xxxhdpi)
- ✅ 다크 모드 스플래시 화면 지원
- ✅ Adaptive 아이콘 지원 (Android 8.0+)
...
1. **다운로드 URL 확인**:
   ```
   https://github.com/EunsuJeong/BS_HR_System/releases/download/v1.0.1/app-debug.apk
   ```
...
1. 업데이트된 QR 코드 생성
2. 직원들에게 새 버전 배포 안내
3. v1.0.0 Release는 유지 (이전 버전)

---

**Release 생성이 완료되면 알려주세요!**
QR 코드를 v1.0.1 버전으로 업데이트하겠습니다.
