...
const StaffNotification = ({ currentUser, getText, selectedLanguage, regularNotifications = [], realtimeNotifications = [] }) => {
  ...
  // 알림 만료 체크 함수 (warningDays 기반, common_admin_notification.js와 동일 로직)
  const isNotificationExpired = (notification) => {
    if (!notification?.createdAt) return true;

    const now = new Date();

    // 연차 알림 특별 처리: warningDays 값에 따라 만료 기준 분기
    if (notification.related?.entity === 'annualLeave') {
      ...
      // warningDays 30일/7일: 연차 종료일까지 표시
      ...
      // warningDays 180일/90일 또는 없음: 생성일 기준 30일간 표시
  ...
  // ✅ DB에서 직접 알림 데이터 로드
  // App.js에서 이미 필터링된 regularNotifications + realtimeNotifications를 props로 받아 사용
  useEffect(() => {
    if (!currentUser?.id) return;

    const allNotifications = [...regularNotifications, ...realtimeNotifications];
    ...
    setEmployeeNotifications(filtered);
  }, [currentUser?.id, regularNotifications, realtimeNotifications]);
